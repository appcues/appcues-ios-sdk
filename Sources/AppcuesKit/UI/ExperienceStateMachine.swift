...
internal class ExperienceStateMachine {
    ...
    func transition(to newState: ExperienceState) {
        ...
        case let (_, .begin(experience)):
            experienceLifecycleEventDelegate.invoke { $0.lifecycleEvent(.experienceError(experience, "experience already active")) }
        case (.empty, _):
    ...
    private func handleStepError(_ experience: Experience, _ errorType: ErrorType, _ message: String) {
        switch errorType {
        case .experience:
            experienceLifecycleEventDelegate.invoke { $0.lifecycleEvent(.experienceError(experience, message)) }
...
extension ExperienceStateMachine: ExperienceContainerLifecycleHandler {
    ...
    func containerDidAppear() {
        ...
        if isFirst {
            experienceLifecycleEventDelegate.invoke { $0.lifecycleEvent(.experienceStarted(experience)) }
            experienceDidAppear()
        }

        experienceLifecycleEventDelegate.invoke { $0.lifecycleEvent(.stepSeen(experience, stepIndex)) }
    }
    ...
    func containerDidDisappear() {
        ...
        case let .endStep(experience, stepIndex, package):
            guard package.wrapperController.isBeingDismissed == true else { return }
            experienceLifecycleEventDelegate.invoke { $0.lifecycleEvent(.stepInteraction(experience, stepIndex)) }
        ...
        case let .renderStep(experience, stepIndex, package, _):
            ...
            if stepIndex == experience.steps.count - 1 {
                experienceLifecycleEventDelegate.invoke { $0.lifecycleEvent(.stepInteraction(experience, stepIndex)) }
                experienceLifecycleEventDelegate.invoke { $0.lifecycleEvent(.stepCompleted(experience, stepIndex)) }
                experienceLifecycleEventDelegate.invoke { $0.lifecycleEvent(.experienceCompleted(experience)) }
            } else {
                experienceLifecycleEventDelegate.invoke { $0.lifecycleEvent(.experienceDismissed(experience, stepIndex)) }
            }
