        ...
        if analytic == .event && value == "request-permission-camera" {
            ...
            if status == .notDetermined {
                AVCaptureDevice.requestAccess(for: .video, completionHandler: { accessGranted in
                    let newStatus: AVAuthorizationStatus = accessGranted ? .authorized : .denied
                    Appcues.shared.identify(userID: userID, properties: ["permissionStatusCamera": "\(newStatus.description)"])
                })
            }
        }
    }
