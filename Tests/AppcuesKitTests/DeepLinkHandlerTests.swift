...
class DeepLinkHandlerTests: XCTestCase {
    ...
    func testHandlePreviewURLWithActiveScene() throws {
        ...
        appcues.experienceLoader.onLoad = { id, published, triggeredBy, completion in
            XCTAssertEqual(id, "f0edab83-5257-47a5-81fc-80389d14905b")
            XCTAssertFalse(published)
            guard case .preview = triggeredBy else { return XCTFail() }
            loaderCalled = true
            completion?(.success(()))
        }
    ...
    func testHandleContentURLWithInactiveScene() throws {
        ...
        appcues.experienceLoader.onLoad = { id, published, triggeredBy, completion in
            ...
            guard case .deepLink = triggeredBy else { return XCTFail() }
            loaderCalled = true
            completion?(.success(()))
        }
